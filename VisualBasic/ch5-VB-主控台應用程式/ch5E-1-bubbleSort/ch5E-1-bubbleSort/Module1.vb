Module Module1
    ...
    Sub Main()
        ...
        '-----------------
        Console.WriteLine("總分統計結果如下: ")
        For i = 0 To animal_num - 1
            For j = 0 To 2
                data_sum(i) = data_sum(i) + data_2d(i, j)
            Next j
            Console.WriteLine(animal_names(i) & ":" & data_sum(i))
        Next i

        '-----------------
        For i = 0 To data_count - 2                              ' 4個動物，只需排3次，最後一個數會自動排好
            one_bubble(data_sum, animal_names, i, data_count)    ' 右邊已排好的不需要再排
            Console.WriteLine("第" & i & "回合結果:")                  ' 印出每回合結果
            For j = 0 To 3
                Console.WriteLine(animal_names(j) & data_sum(j))       '將分數連同動物名字印出來 
            Next j
        Next i
        ...
        ' 印出前三名
        For i = 0 To 2
            Console.WriteLine("第" & i + 1 & "名:" & animal_names(data_count - i - 1))  '分數最高的排在最右邊，因此要用data_count去扣掉i才能取得前三大的
        Next i
        Console.Read()
        '======== 主程式結束 ============================================================
    End Sub
